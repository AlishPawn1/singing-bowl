...
jQuery(function ($) {
    ...
    $(document).ready(function () {
        const formsConfig = [
            {
                formId: "#order-select",
                dropdownClass: ".order-dropdown",
                dropdownWrapperClass: "order-select-dropdown",
                selectedClass: "order-select-dropdown-select",
                menuClass: "order-select-dropdown-menu",
                menuItemClass: "order-select-dropdown-menu-item",
                search: false
            },
            {
                formId: "#checkout-country-form",
                dropdownClass: ".checkout-country-dropdown",
                dropdownWrapperClass: "country-form-dropdown",
                selectedClass: "country-form-dropdown-select",
                menuClass: "country-form-dropdown-menu",
                menuItemClass: "country-form-dropdown-menu-item",
                search: true,
            ...
            {
                formId: "#search-category",
                dropdownClass: ".select-search-category",
                dropdownWrapperClass: "search-category",
                selectedClass: "search-category-select",
                menuClass: "search-category-menu",
                menuItemClass: "search-category-menu-item",
                search: false
            }
        ];
        ...
        formsConfig.forEach(config => {
            ...
            const dropdowns = $(config.dropdownClass);
    
            dropdowns.each(function () {
                createCustomDropdown($(this), config);
            });
        ...
        function createCustomDropdown(dropdown, config) {
            const options = dropdown.find("option");
            const customDropdown = $(`<div class='${config.dropdownWrapperClass} position-relative'></div>`);
            dropdown.after(customDropdown);
    
            const selected = $(`<div class='${config.selectedClass}'></div>`).text(options.eq(0).text());
            customDropdown.append(selected);
    
            const menu = $(`<div class='${config.menuClass}'></div>`);
            customDropdown.append(menu);
    
            if (config.search) {
                const search = $(`<input class='${config.menuClass}-search' placeholder='${config.searchPlaceholder}'>`);
                menu.append(search);
                search.on("input", function () {
                    filterItems(options, menu, search);
                });
            ...
            const menuInnerWrapper = $("<div class='menu-inner'></div>");
            menu.append(menuInnerWrapper);
    
            options.each(function (index, option) {
                const item = $(`<div class='${config.menuItemClass}'></div>`)
                    .data("value", $(option).val())
                    .text($(option).text());
                menuInnerWrapper.append(item);
            ...
            selected.on("click", function () {
                menu.toggle();
                selected.toggleClass('open', menu.is(':visible'));
                if (config.search) {
                    menu.find("input").focus();
                }
            });
            ...
            $(document).on("click", function (e) {
                if (!$(e.target).closest(`.${config.dropdownWrapperClass}`).length && menu.is(":visible")) {
                    menu.hide();
                    selected.removeClass('open');
                }
            });
        ...
        function setSelected(selected, dropdown, menu, item) {
            ...
            item.addClass("is-select").siblings().removeClass("is-select");
            menu.find("input").val("");
            menu.find(".menu-inner div").show();
        }
    
        function filterItems(options, menu, search) {
            const customOptions = menu.find(".menu-inner div");
            const value = search.val().toLowerCase();
